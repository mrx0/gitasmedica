...
//move_price2_items_in_cat_f.php
...
//                    $expires_time = date_format($expires_time, 'Y-m-d');
                    ...
                    if ($_POST['item_id'] != 0) {
                        $query = "UPDATE `spr_price2_items` SET `parent_id`='{$_POST['target_cat_id']}' WHERE `id`='{$_POST['item_id']}'";
                    ...
                    if ($_POST['cat_id'] != 0) {
                        ...
                        $query = "SELECT `id`, `node_count` FROM `spr_price2_category` WHERE `id` iN (SELECT `parent_id` FROM `spr_price2_category` WHERE `id`='{$_POST['cat_id']}')";
                        ...
                        if ($parent_id_now != $_POST['target_cat_id']) {
                            ...
                            if (!checkExistTreeParents ('spr_price2_category', $_POST['cat_id'], $_POST['target_cat_id'])) {

                                //Если бывший родитель указан и это не корень (0)
                                if ($parent_id_now != 0) {

                                    //Обновляем кол-во подкатегорий (node_count)
                                    $query = "UPDATE `spr_price2_category` SET `node_count`=`node_count`-1 WHERE `id` = '$parent_id_now'";
                                    $res = mysqli_query($msql_cnnct, $query) or die(mysqli_error($msql_cnnct) . ' -> ' . $query);
                                }

                                //Обновляем подкатегорию
                                $query = "UPDATE `spr_price2_category` SET `parent_id`='{$_POST['target_cat_id']}' WHERE `id`='{$_POST['cat_id']}'";
                                $res = mysqli_query($msql_cnnct, $query) or die(mysqli_error($msql_cnnct) . ' -> ' . $query);

                                //Обновляем кол-во подкатегорий (node_count)
                                $query = "UPDATE `spr_price2_category` SET `node_count`=`node_count`+1 WHERE `id`='{$_POST['target_cat_id']}'";
                                $res = mysqli_query($msql_cnnct, $query) or die(mysqli_error($msql_cnnct) . ' -> ' . $query);
