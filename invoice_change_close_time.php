			...
			if ($_GET){
				...
				if ($invoice_j !=0){
					echo '
						<div id="status">
							<header>
								<h2>Изменить дату закрытия работы по наряду <a href="invoice.php?id='.$_GET['invoice_id'].'" class="ahref">#'.$_GET['invoice_id'].'</a></h2>';
                    echo '				
				            </header>';
                        ...
                        echo '
                                    ...
                                    <div class="filterBlock">
                                        ...
                                        <div class="filtercellRight" style="width: 245px; min-width: 245px;">
                                            <input type="text" id="datanew" name="datanew" class="dateс" value="' . date('d.m.Y', strtotime($invoice_j[0]['closed_time'])) . '" onfocus="this.select();_Calendar.lcs(this)"
                                            onclick="event.cancelBubble=true;this.select();_Calendar.lcs(this)">
                                        </div>
                                    </div>
                                    
                                    <input type="hidden" id="id" name="id" value="' . $_GET['invoice_id'] . '">
                                    <div id="errror"></div>
                                    <input type="button" class="b" value="Применить" onclick="Ajax_invoice_close_time_edit(' . $_GET['invoice_id'] . ')">
