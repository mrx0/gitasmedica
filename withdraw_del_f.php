...
//Функция для Удаление
	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST){
            ...
            $withdraw_j = SelDataFromDB('journal_withdraw', $_POST['id'], 'id');

            //Если заднее число
            if ((strtotime($withdraw_j[0]['create_time']) + 12*60*60 < time()) && (($finances['see_all'] != 1) && !$god_mode)){
                echo '
...
//                } else {
                    ...
                    $query = "DELETE FROM `journal_withdraw` WHERE `id`='{$_POST['id']}'";
