...
var _Calendar=function(){
	var _Calendar={
		...
		cs_click:function(e){
			//то, что стало (новое значение)
			//console.log(_Calendar.calvalarr[_Calendar.evtTgt(_Calendar.EvtObj(e)).id.substring(2,_Calendar.evtTgt(_Calendar.EvtObj(e)).id.length)]);

			//Было --> стало
            //console.log(curdt +' --> '+ _Calendar.calvalarr[_Calendar.evtTgt(_Calendar.EvtObj(e)).id.substring(2,_Calendar.evtTgt(_Calendar.EvtObj(e)).id.length)]);
			if (curdt != _Calendar.calvalarr[_Calendar.evtTgt(_Calendar.EvtObj(e)).id.substring(2,_Calendar.evtTgt(_Calendar.EvtObj(e)).id.length)]) {
                doSomeThingAfterChangeCalendar(_Calendar.updobj.getAttribute("class").split(" "), _Calendar.updobj.id, curdt, _Calendar.calvalarr[_Calendar.evtTgt(_Calendar.EvtObj(e)).id.substring(2, _Calendar.evtTgt(_Calendar.EvtObj(e)).id.length)]);
            }
		...
		prepcalendar:function( hd, cm, cy ){
			...
			//До какого года выводить список
			var endYear = 1;
		...
		today:function(){
            ...
            //Было --> стало
            // console.log(curdt +' --> '+ _Calendar.addnull(_Calendar.now.getDate(),_Calendar.now.getMonth()+1,_Calendar.now.getFullYear()));
            // console.log(_Calendar.updobj.id);
            // console.log(_Calendar.updobj.getAttribute("class").split(" "));
            if (curdt != _Calendar.addnull(_Calendar.now.getDate(),_Calendar.now.getMonth()+1,_Calendar.now.getFullYear())) {
                doSomeThingAfterChangeCalendar(_Calendar.updobj.getAttribute("class").split(" "), _Calendar.updobj.id, curdt, _Calendar.addnull(_Calendar.now.getDate(), _Calendar.now.getMonth() + 1, _Calendar.now.getFullYear()));
            }
	...
	document.write('<table id="fc" style="position: absolute; border-collapse:collapse; background:#FFFFFF; border:1px solid #303030;display:none; -moz-user-select:none; -khtml-user-select:none; user-select:none; z-index: 105;" cellpadding=2>');
