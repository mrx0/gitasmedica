...
//!!! доделать сравнение времени, учитывая месяц и тд и тп
	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST){
			...
			if (!isset($_POST['client_id']) || !isset($_POST['summ']) || !isset($_POST['invoice_id']) || !isset($_POST['filial_id']) || !isset($_POST['date_in'])){
                                                    ...
                                                    if (($client_balance['summ'] <= 0) || ($client_balance['summ'] - $client_balance['debited'] - $client_balance['withdraw'] + $client_balance['refund'] <= 0)) {
                                                        ...
                                                        if ($payed > $invoice_j[0]['summ']) {
                                                            ...
                                                            if ($debited > $client_balance['summ']) {
                                                                echo json_encode(array('result' => 'error', 'data' => '<div class="query_neok">На балансе не хватит средств для оплаты</div>', 'data1' => $debited, 'data2' => $client_balance['summ']));
                                                            } else {
