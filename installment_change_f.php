...
//Функция для изменения статуса рассрочки
	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST){
		    if (isset($_POST['installment_id']) && isset($_POST['client_id']) && isset($_POST['invoice_id']) && isset($_POST['installment_status_now'])) {
...
//                ];
                ...
                //$query = "UPDATE `journal_installments` SET `status`= :status WHERE `client_id`= :client_id AND `invoice_id`= :invoice_id";
                $query = "UPDATE `journal_installments` SET `status`= :status WHERE `id`= :installment_id";
