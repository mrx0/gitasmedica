...
//Все поздравления с ДР
	...
	if ($enter_ok){
		...
		if (($scheduler['see_all'] == 1) /*|| ($scheduler['see_own'] == 1)*/ || $god_mode){
            ...
            $query = "
                    SELECT s_w.id, s_w.name, s_w.birth, j_bc.status
                    FROM `spr_workers` s_w
                    LEFT JOIN `journal_bd_congr` j_bc
                    ON s_w.status <> '8' AND j_bc.worker_id = s_w.id AND j_bc.year = '$year'
                    WHERE s_w.birth <> '0000-00-00'";
...
//            var_dump($birthdays_arr);
            ...
            echo '
                                <li class="cellsBlock" style="font-weight: bold; width: auto; text-align: right; margin-bottom: 10px;">
                                    <span style="font-size: 90%; color: rgb(125, 125, 125);">Сегодня: <a href="all_congrats.php" class="ahref">'.date("d").' '.$monthsName[date("m")].' '.date("Y").'</a></span>
                                </li>
                                <li class="cellsBlock" style="font-weight: bold; width: auto; text-align: right;">
                                    <a href="all_congrats.php?y='. ($year - 1) .'" class="cellTime ahref" style="text-align: center;">
                ...
                for ($i = 1; $i <= 12; $i++) {
                    ...
                    for ($d = $start; $d <= $end; $d++) {
                        ...
                        if (array_key_exists(dateTransformation($i) . "-" . sprintf("%02d", $d), $birthdays_arr)) {
                            //var_dump($birthdays_arr[dateTransformation($i) . "-" . sprintf("%02d", $d)]);
                            foreach ($birthdays_arr[dateTransformation($i) . "-" . sprintf("%02d", $d)] as $bd_date){
                                $bd_title .= $bd_date['name'].' ';

                                if ($bd_date['status'] == 1){
                                    $bd_title .= '[+]; ';
                                    if ($bd_congr) {
                                        $bd_bg_color = 'background-color: rgba(255, 255, 60, 0.33);';
                                    }
                                }else{
                                    $bd_title .= '[ - НЕ ПОЗДРАВИЛИ - ]; ';
                                    $bd_bg_color = 'background-color: rgba(10, 10, 10, 0.10);';
                                    $bd_congr = FALSE;
                                }

                            }
                            //$day_marks = '<div style="position: absolute; color: red; font-size: 70%; top: -8px; right: -4px;" title="'.$bd_title.'">&#9679;</div>';
                        }


                        echo '
                                        <td class="cellsBlockHover" style="' . $today_color . ' '.$bd_bg_color.' text-align: center; text-align: -moz-center; text-align: -webkit-center; vertical-align: top;" title="'.$bd_title.'">';
                        if ($d < 1 || $d > $day_count) {
                            echo "&nbsp";
                        } else {
