...
//ajax_show_result_stat_stom4_f.php
//
		...
		if ($_POST){
			...
			if ($workerExist){
				...
				if ($queryDopExist){
					...
					//"SELECT * FROM `journal_tooth_status` WHERE `create_time` BETWEEN '1451592000' AND '1476993599' AND`worker` = '398'"
					
					$query = $query." GROUP BY `client` ORDER BY `create_time` DESC";
					//var_dump($query);
					
					$queryGroup .= "SELECT * FROM (SELECT * FROM `journal_tooth_status` ORDER BY `create_time` DESC) AS `stom`";
					if ($queryDopExist){
						$queryGroup .= '
						WHERE '.$queryDop;
						$queryDopExist = true;
					...
					$res = mysql_query($queryGroup) or die(mysql_error().' -> '.$queryGroup);
							...
							if (($i > 0) && ($journal[$i]['client'] == $journal[$i-1]['client'])){
								...
								//$journal_count_orig++;
									...
									//if (($journal[$i]['create_time'] >= $datestart)  && ($journal[$i]['create_time'] <= $datefinish)){
										...
										if (missingTeeth($journal[$i]['id'], $t_f_data, $cl_age)){
										}else{
											//!!! перенес сверху сюда
											$journal_count_orig++;
											echo '
