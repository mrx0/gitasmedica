...
//
	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST){
			...
			if (!isset($_POST['insure']) || !isset($_POST['client']) || !isset($_POST['zapis_id']) || !isset($_POST['filial']) || !isset($_POST['worker'])){
					...
					foreach ($data as $zub => $invoice_data){
						...
						if (!empty($invoice_data)){
							...
							foreach ($invoice_data as $key => $items){
								...
								$_SESSION['invoice_data'][$_POST['client']][$_POST['zapis_id']]['data'][$zub][$key]['insure'] = $_POST['insure'];
