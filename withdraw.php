...
//withdraw.php
//Выдача денег со счёта пациенту
        ...
        if (($finances['see_all'] == 1) || ($finances['see_own'] == 1) || $god_mode){
            ...
            if ($_GET){
                if (isset($_GET['id'])){
                    ...
                    $refund_j = SelDataFromDB('journal_withdraw', $_GET['id'], 'id');
                        ...
                        echo '
                            <div id="status">
								<header>
									...
									<h2>Выдача #'.$_GET['id'].' от '.date('d.m.y' ,strtotime($refund_j[0]['date_in'])).'';
...
//                                echo '
//                                        <a href="#" onclick="Ajax_reopen_refund('.$_GET['id'].', '.$refund_j[0]['client_id'].')" title="Разблокировать" class="info" style="font-size: 100%;"><i class="fa fa-reply" aria-hidden="true"></i></a><br>';
//                            }
                        }
...
//                        }
                        ...
                        echo '
                                <ul style="margin-left: 6px; margin-bottom: 10px;">
                                    <li style="font-size: 85%; color: #7D7D7D; margin-bottom: 5px;">
                                        Контрагент: '.WriteSearchUser('spr_clients',  $refund_j[0]['client_id'], 'user_full', true).'
                                    </li> 
                                    <li style="font-size: 85%; color: #7D7D7D; margin-bottom: 5px;">
                        ...
                        if (($refund_j[0]['create_time'] != 0) || ($refund_j[0]['create_person'] != 0)){
                            echo '
											Добавлен: '.date('d.m.y H:i' ,strtotime($refund_j[0]['create_time'])).'<br>
											Автор: '.WriteSearchUser('spr_workers', $refund_j[0]['create_person'], 'user', true).'<br>';
                        }else{
                            echo 'Добавлен: не указано<br>';
                        }
                        if (($refund_j[0]['last_edit_time'] != 0) || ($refund_j[0]['last_edit_person'] != 0)){
                            echo '
											Последний раз редактировался: '.date('d.m.y H:i' ,strtotime($refund_j[0]['last_edit_time'])).'<br>
											Кем: '.WriteSearchUser('spr_workers', $refund_j[0]['last_edit_person'], 'user', true).'';
                        }
                        echo '
										</span>
									</div>';
							...
							<div class="cellsBlock2">
								<div class="cellRight">
								    <ul style="margin-left: 6px; margin-bottom: 10px;">
                                        ...
                                        <li style="margin-bottom: 5px; font-size: 110%; font-weight: bold; color: red;">
									        '.$refund_j[0]['summ'].' руб.
									    </li>
							        </ul>
								</div>
								...
								<div class="cellRight">
								    <ul style="margin-left: 6px; margin-bottom: 10px;">
                                        ...
                                        <li style="font-size: 90%; margin-bottom: 5px;">
                                            ', $refund_j[0]['summ_type'] == 1 ? 'Наличный' : 'Безналичный' ,'
									    </li>
								    </ul>
								</div>
							</div>';
