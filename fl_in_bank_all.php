...
//                    }
                ...
                $query = "SELECT * FROM `fl_journal_in_bank` WHERE `filial_id`='$filial_id' AND `year`= '$year' AND `month`='$month'
                    ORDER BY `day` ASC";
                //var_dump($query);
                ...
                echo '
                    <div id="status">
                        <header>
                            ...
                            <h2>Все, что было добавлено в банк</h2>
                            <a href="fl_in_bank_add.php" class="b">Добавить новый</a>
                        </header>';
                ...
                echo widget_calendar ($month, $year, 'fl_in_bank_all.php', $dop);
                echo '</div>';
                ...
                if (!empty($rezult)){
                    ...
                    foreach ($rezult as $item){
                        ...
                        if ( $item['status'] != 9) {
                            $result_temp .= ' 
                                    <div class="cellCosmAct info" style="font-size: 100%; text-align: center; border-top: none;" onclick="fl_deleteInBank(' . $item['id'] . ');">
                                        <i class="fa fa-times" aria-hidden="true" style="cursor: pointer;"  title="Удалить"></i>
                                    </div>';
                        }else {
                            $result_temp .= '
                                    <div class="cellCosmAct info" style="font-size: 100%; text-align: center; border-top: none;" onclick="fl_reopenInBank('.$item['id'].');">
