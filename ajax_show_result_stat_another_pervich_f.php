...
//ajax_show_result_stat_another_pervich_f.php
//Функция для Первичка v3.0. Поиск тех, кто был первичкой, но потом пришёл
...
//                    "
                    ...
                    $query = "
                        SELECT jiex.*, ji.summ AS invoice_summ, ji.client_id AS client_id, ji.summins AS invoice_summins, ji.status AS invoice_status, ji.type AS type, ji.zapis_id AS zapis_id, z.enter AS enter, z.pervich AS pervich, sc.birthday2 AS birthday, CONCAT_WS('-', z.year, LPAD(z.month, 2, '0'), LPAD(z.day, 2, '0')) AS z_date
                        FROM `zapis` z
...
//                            if (!in_array($arr['client_id'], $client_ids)){
//                                array_push($client_ids, $arr['client_id']);
//                            }
