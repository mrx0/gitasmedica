		...
		if (($zapis['see_all'] == 1) || ($zapis['see_own'] == 1) || $god_mode){
			...
			if ($_POST){
				...
				echo '				
                    ...
                    echo '
											<select id="labor" class="wrapper-dropdown-2 b2" tabindex="2" name="filial">
												<ul class="dropdown">
													<li><option value="0" selected>Все</option></li>';
														if ($labor_j !=0){
															for ($i = 0; $i < count($labor_j); $i++){
																echo '<li><option value="'.$labor_j[$i]['id'].'" class="icon-twitter icon-large">'.$labor_j[$i]['name'].'</option></li>';
															}
														}
                ...
                if (!empty($filials_j)){
                    foreach ($filials_j as $f_id => $f_item){
                        echo '<li><option value="'.$f_id.'" class="icon-twitter icon-large">'.$f_item['name'].'</option></li>';
                    }
                }
...
//										<input type="checkbox" id="zapisAll" name="zapisAll" class="zapisType" value="1"> Созданны и не отправленны<br>
//										<input type="checkbox" id="zapisArrive" name="zapisArrive" class="zapisType" value="1" checked> Отправлены в лаб.<br>
//										<input type="checkbox" id="zapisNotArrive" name="zapisNotArrive" class="zapisType" value="1"> Вернулись из лаб.<br>
//										<input type="checkbox" id="zapisNull" name="zapisNull" class="zapisType" value="1"> Все открытые<br>
//										<input type="checkbox" id="zapisError" name="zapisError" class="zapisType" value="1"> Отменены<br>
//									</div>
//								</li>';
				...
				echo '
							...
							<ul style="border: 1px dotted #CCC; margin: 10px; padding: 10px 15px 20px; width: 420px; font-size: 95%; background-color: rgba(245, 245, 245, 0.9);display: inline-table;">
								...
								<li class="filterBlock">
									...
									<div class="filtercellRight" style="width: 245px; min-width: 245px;">
										<!--<input id="status" name="status" value="0" type="radio" checked> Все<br>-->
										<input id="status" name="status" value="1" type="radio" checked> Созданные и не отправленные<br>
										<input id="status" name="status" value="2" type="radio"> Отправленые в лаб.<br>
										<input id="status" name="status" value="3" type="radio"> Вернулись из лаб.<br>
										<input id="status" name="status" value="4" type="radio" checked> Все открытые<br>
										<input id="status" name="status" value="5" type="radio"> Отменены<br>
									</div>
								</li>
				...
				echo '
                        <div class="no_print"> 
						<input type="button" class="b" value="Применить" onclick="Ajax_show_result_stat_lab_order()">
						</div>';
