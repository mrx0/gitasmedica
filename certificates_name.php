	...
	if ($enter_ok){
		...
		if (($finances['see_all'] == 1) || ($finances['see_own'] == 1) || $god_mode) {
            ...
            if (($finances['add_new'] == 1) || ($finances['add_own'] == 1) || $god_mode) {
                echo '
					<a href="cert_name_add.php" class="b">Добавить</a>';
            }
            ...
            $cert_j = array();
            //$cert_j = SelDataFromDB('journal_cert_name', '', $limit_pos);
            //var_dump ($cert_j);
            $query = "SELECT * FROM `journal_cert_name` {$dop} ORDER BY `num` ASC LIMIT {$limit_pos[0]}, {$limit_pos[1]}";

            if (isset($_GET['option'])) {
                if ($_GET['option'] == 2) {
                    $query = "SELECT * FROM `journal_cert_name` {$dop} ORDER BY `expires_time` ASC LIMIT {$limit_pos[0]}, {$limit_pos[1]}";
                }
                if ($_GET['option'] == 3) {
                    $query = "SELECT * FROM `journal_cert_name` {$dop} ORDER BY `expires_time` DESC LIMIT {$limit_pos[0]}, {$limit_pos[1]}";
                }
                if ($_GET['option'] == 5) {
                    $query = "SELECT * FROM `journal_cert_name` {$dop} ORDER BY `expires_time` DESC LIMIT {$limit_pos[0]}, {$limit_pos[1]}";
                }
                if ($_GET['option'] == 4) {
                    $query = "SELECT * FROM `journal_cert_name` {$dop} ORDER BY `closed_time` DESC LIMIT {$limit_pos[0]}, {$limit_pos[1]}";
                }
            }
//            var_dump($query);
