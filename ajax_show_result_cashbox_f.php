...
//
	...
	if (empty($_SESSION['login']) || empty($_SESSION['id'])){
		...
		if ($_POST) {
            ...
            /*var_dump($datastart);
            var_dump(strtotime($_POST['dataend'].' 23:59:59'));
            var_dump(time());*/

            if (($_POST['datastart'] != $_POST['dataend']) && (($finances['see_all'] != 1) && !$god_mode)){
                echo '<span style="color: red;">Вы можете смотреть данные только за одну определённую дату</span>';
            }else {
                ...
                $rezult_solar = $rezult_temp['rezult_solar'];
                $rezult_realiz = $rezult_temp['rezult_realiz'];
                ...
                if (!empty($rezult) || !empty($rezult_cert) || !empty($rezult_abon) || !empty($rezult_solar) || !empty($rezult_realiz)) {
                    ...
                    foreach ($rezult as $order_item) {
                        ...
                        if (isset($filialResult[$order_item['office_id']])) {
                            ...
                            $filialResult[$order_item['office_id']]['data_solar'] = array();
                            $filialResult[$order_item['office_id']]['data_realiz'] = array();
                    ...
                    foreach ($rezult_abon as $order_item) {
                        ...
                        if (isset($filialResult[$order_item['filial_id']])) {
                            ...
                            $filialResult[$order_item['filial_id']]['data_solar'] = array();
                            $filialResult[$order_item['filial_id']]['data_realiz'] = array();
                    ...
                    //Солярий
                    foreach ($rezult_solar as $order_item) {
                        //var_dump($order_item['office_id']);
                        //var_dump(isset($filialResult[$order_item['office_id']]));
                        if (isset($filialResult[$order_item['filial_id']])) {
                            array_push($filialResult[$order_item['filial_id']]['data_solar'], $order_item);
                        } else {
                            $filialResult[$order_item['filial_id']]['data'] = array();
                            $filialResult[$order_item['filial_id']]['data_cert'] = array();
                            $filialResult[$order_item['filial_id']]['data_abon'] = array();
                    ...
                    //Реализация
                    foreach ($rezult_realiz as $order_item) {
                        //var_dump($order_item['office_id']);
                        //var_dump(isset($filialResult[$order_item['office_id']]));
                        if (isset($filialResult[$order_item['filial_id']])) {
                            array_push($filialResult[$order_item['filial_id']]['data_realiz'], $order_item);
                        } else {
                            $filialResult[$order_item['filial_id']]['data'] = array();
                            $filialResult[$order_item['filial_id']]['data_cert'] = array();
                            $filialResult[$order_item['filial_id']]['data_abon'] = array();
                    ...
                    foreach ($filialResult as $filialID => $filialData) {
                        ...
                        //Солярий
                        if (!empty($filialData['data_solar'])) {
                            foreach ($filialData['data_solar'] as $order_item) {
                                ...
                                $orderTemp_str .= '
                                                    <li class="cellsBlock" style="width: auto;">';
                                $orderTemp_str .= '
                                                        <a href="abonement.php?id=' . $order_item['id'] . '" class="cellOrder ahref" style="position: relative; pointer-events: none;">
                                                            <b>Посещение солярия #' . $order_item['id'] . '</b>  от ' . date('d.m.y', strtotime($order_item['date_in'])) . '<br>
                                                            ' . $office_j_arr[$order_item['filial_id']]['name'] . '<br>
                                                            <span style="font-size:80%;  color: #555;">';
                        ...
                        //Реализация
                        if (!empty($filialData['data_realiz'])) {
                            foreach ($filialData['data_realiz'] as $order_item) {
                                ...
                                $orderTemp_str .= '
                                                        <a href="abonement.php?id=' . $order_item['id'] . '" class="cellOrder ahref" style="position: relative; pointer-events: none;">
                                                            <b>Реализация #' . $order_item['id'] . '</b>  от ' . date('d.m.y', strtotime($order_item['date_in'])) . '<br>
                                                            ' . $office_j_arr[$order_item['filial_id']]['name'] . '<br>
                                                            <span style="font-size:80%;  color: #555;">';
                        ...
                        if ((strlen($orderClose_str) > 1) && (($finances['see_all'] != 0) || $god_mode)) {
                            ...
                            $filialData_str .= '<li style="font-size: 85%; color: #7D7D7D; margin-bottom: 5px; height: 30px; ">Удалённые из программы документы</li>';
