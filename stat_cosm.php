						...
						if ($number != 0){
							...
							for ($k = 0; $k < count($actions_cosmet)-2; $k++) { 
								if ($actions_cosmet[$k]['active'] != 0){
									...
									$query = "SELECT * FROM `journal_cosmet1` WHERE {$filter_rez[1]} AND `worker`={$value} AND `office`={$value1} AND `status` <> '9' AND `c{$actions_cosmet[$k]['id']}`='1' AND `status` <> '9' ORDER BY `create_time` DESC";
									$res = mysql_query($query) or die($query);
									$number = mysql_num_rows($res);
									if ($number != 0){
										$с1 = $number;
									...
									if ($actions_cosmet[$k]['id'] == 13){
										...
										$query = "SELECT * FROM `journal_cosmet1` WHERE {$filter_rez[1]} AND `worker`={$value} AND `office`={$value1} AND `status` <> '9' AND `c{$actions_cosmet[$k]['id']}`='1' AND `c13`='1' ORDER BY `create_time` DESC";
									}
									//var_dump ($query);
									$res = mysql_query($query) or die($query);
									$number = mysql_num_rows($res);
