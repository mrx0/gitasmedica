			...
			if (!isset($_POST['invoice_type']) || !isset($_POST['summ']) || !isset($_POST['summins']) || !isset($_POST['client']) || !isset($_POST['date_in']) || !isset($_POST['filial']) || !isset($_POST['worker'])){
                ...
                if ($_POST['client'] != '') {
                    ...
                    if ($clientSearch == 0) {
                        ...
                        $worker = SelDataFromDB ('spr_workers', $_POST['worker'], 'worker_full_name');
                        if ($worker == 0){
                            echo json_encode(array('result' => 'error', 'data' => '<span style="color: red;">Не найден исполнитель.</span>'));
                        }else{
