...
//                    if (($fast_search[$i]['cell_time'] != '0000-00-00 00:00:00') && ($fast_search[$i]['status'] == 7)) {
                        ...
                        if (!$expired && !$debited) {
                            if ($fast_search[$i]['client_id'] != 0) {
                                $rez .= "<td style = 'background-color: rgb(207 255 207); text-align: center; cursor: pointer;' onclick = 'Ajax_cert_name_add_pay(".$fast_search[$i]['id'].")' ><i class='fa fa-check' aria - hidden = 'true' ></i ></td >";
                            }else{
