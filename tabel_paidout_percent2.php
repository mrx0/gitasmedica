...
if (empty($_SESSION['login']) || empty($_SESSION['id'])){
    ...
    if ($_POST) {
        if (isset($_POST['tabel_id']) && isset($_POST['summ'])){
            ...
            echo '
                ...
                <!--<span style="font-size: 85%;">Наряды (Филиал)</span><br>-->';
            //var_dump($invoices_j);
...
//                                ' . $invoice_id . ' (' . $filials_j[$invoice_item['filial_id']]['name2'] . ')
//                            </a>';
...
//Сумма которую осталось выдать !!! уточнить переделать
$summItog = intval($tabel_j['summ_calc']);

//Часть, которую хотим выдать
$iWantMyMoney = $_POST['summ'];

//Суммы выплат, которые у нас уже были по этому табелю по филиалам
$paidOutsFilialsPrev = array();
//Соберём их
$query = "SELECT * FROM `fl_journal_tabels_paidouts` WHERE `tabel_id`='{$tabel_id}';";
...
if ($number != 0){
    while ($arr = mysqli_fetch_assoc($res)) {
        ...
        //Сразу добавляем в итоговый массив,
        //предварительно добавив в массив элемент с ID филиала, если его не было
        if (!isset($paidOutsFilialsPrev[$arr['filial_id']])) {
            $paidOutsFilialsPrev[$arr['filial_id']] = 0;
        }
        $paidOutsFilialsPrev[$arr['filial_id']] = $arr['summ'];
    }
}
//var_dump($paidOutsFilialsPrev);



if (!empty($paidOutsFilialsPrev)){
    echo 'Сколько с каких филиалов уже было выдано по этом табелю<br><br>';

    foreach ($paidOutsFilialsPrev as $filial_id => $summ){
        echo $filials_j[$filial_id]['name2'].' -> '.$paidOutsFilialsPrev[$filial_id];
    }
}else{
    echo 'По этому табелю еще ничего не выдавалось<br><br>';
}


echo '<br><br>Всего осталось выдать: '.$summItog.'<br><br><br>';
...
echo 'Сколько с каких филиалов надо выдать<br><br>';
foreach ($itog_filials_percents as $filial_id => $percent) {
    $summ4ZP_temp[$filial_id] = $summItog / 100 * $percent;

    echo $filials_j[$filial_id]['name2'].' -> '.$summ4ZP_temp[$filial_id].'<br>';
}
...
//Части по филиалам, которые хотим выдать СЕЙЧАС
$summ4ZPNow = array();

echo 'Мы хотим выдать сейчас: '.$iWantMyMoney.'<br><br>';
echo 'Сколько с каких филиалов будем выдавать<br>';
foreach ($itog_filials_percents as $filial_id => $percent) {
    $summ4ZPNow[$filial_id] = $iWantMyMoney / 100 * $percent;

    echo $filials_j[$filial_id]['name2'].' -> '.$summ4ZPNow[$filial_id].'<br>';
}
//var_dump($summ4ZPNow);
            ...
            foreach ($filials_j as $f_id => $filials_j_data) {
                ...
                if (isset($summ4ZPNow[$f_id])){
                    $value = $summ4ZPNow[$f_id];
                }
            ...
            $iWantMyMoney2 = $iWantMyMoney1 - $summPaidOutsPrev;
            var_dump($iWantMyMoney2);
