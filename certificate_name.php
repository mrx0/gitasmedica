...
//certificate_name.php
//карточка именного сертификата
				...
				if ($cert_j != 0){
					...
					if (($spravka['edit'] == 1) || $god_mode){
						if ($cert_j[0]['status'] != 9){
							echo '
										<a href="cert_name_edit.php?id='.$_GET['id'].'" class="info" style="font-size: 80%;" title="Редактировать"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>';
						}
...
//                            </div>';
                    ...
                    if ($cert_j[0]['status'] != 9){
                        ...
                        if (($cert_j[0]['status'] != 7) && ($cert_j[0]['cell_time'] == '0000-00-00 00:00:00')){
                                echo '
                                <a href="cert_name_cell.php?cert_id=' . $cert_j[0]['id'] . '" class="b">Выдать сертификат</a>';
                        }else{
