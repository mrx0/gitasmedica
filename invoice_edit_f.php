...
//                            }
                            ...
                            if ($_POST['cert_name_id'] == $_POST['cert_name_old_id']){
                                ...
                                if ($_POST['cert_name_old_id'] > 0){
                                    ...
                                    $client_balance_summ = 0;

                                    if (!empty($cert_name)){
                                        if ($cert_name['client_id'] > 0){
                                            //работаем с балансом и доступными средствами
                                            //Баланс контрагента
                                            include_once 'ffun.php';
                                            $client_balance = json_decode(calculateBalance($cert_name['client_id']), true);

                                            $client_balance_summ = $client_balance['summ'] - $client_balance['debited'] - $client_balance['withdraw'] + $client_balance['refund'];
                                        }
                                    }
                                    //Если хватает денег на счету
                                    if ($client_balance_summ >= $cert_name['nominal']) {
                                        ...
                                        //Удаляем деньги со счета хозяина (системный ордер по сертификату)
                                        ...
                                        $invoice_can_edit = FALSE;
                                    }

                                //Старого не было, просто добавляем новый
                                }else{
//                                    //Проверяем, можно ли удалить старый
//                                    //Если можно, удаляем старый (точнее убираем у него привязку от наряда), удаляем деньги со счета хозяина
//                                    //Сначала получим данные этого сертификата
...
//                                    }
                            ...
                            if ($invoice_can_edit) {
                                ...
                                */
                                ...
                                echo json_encode(array('result' => 'error', 'data' => 'Нельзя удалить/заменить этот сертификат. На счету не хватает средств.'));
                            }
						}
					}
				}else{
					echo json_encode(array('result' => 'error', 'data' => 'Что-то пошло не так'));
				}
			}
		}
	}
